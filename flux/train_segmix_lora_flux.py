...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if not os.path.exists(self.instance_data_dir):
            raise ValueError("instance_data_dir is not found.")
        self.instance_data_dir_config = os.path.join(instance_data_dir, "metadata.jsonl")
        if not os.path.exists(self.instance_data_dir_config):
            raise ValueError("metadata.jsonl file is not found in the instance_data_dir.")
        
        # not yet implemented
        self.class_config_path = class_data_root
        self.soft_alpha = soft_alpha

        if args.dataset_name is not None:
            raise NotImplementedError("Loading data from datasets is not supported yet.")

        else:
            instance_dict_list = []
            with open(self.instance_data_dir_config, 'r') as f:
                for line in f:
                    try:
            ...
            instance_image_captions = []
            for instance_dict in instance_dict_list[1:]:
                img_path = instance_dict[image_column]
                img = Image.open(os.path.join(self.instance_data_dir, img_path))
                if not img.mode == "RGB":
                    img = img.convert("RGB")
                caption = instance_dict["text"]
                mask_path = instance_dict[mask_column]
                mask = Image.open(os.path.join(self.instance_data_dir, mask_path))
                category = instance_dict[id_column] # 'a'
                self.category_instance_images[category].append({"image":img, "mask":mask})
                instance_image_captions.append((img, mask, caption, category))

        self.instance_image_captions = []
        for img, mask, caption, category in instance_image_captions:
            self.instance_image_captions.extend(itertools.repeat((img, mask, caption, category), repeats))
        ...
        self.num_instance_images = len(self.instance_image_captions)
        self._length = self.num_instance_images

        if self.class_config_path is not None:
            raise NotImplementedError("Loading data from classes is not supported yet.")
        ...
        self.segmix_template = "a close out photo of a small {} and a small {}, {} simple background, pasted, sticker, unnatural, glued, clip-art, attached, collage, low quality."
        self.segmix_prob = segmix_prob
        self.prior_segmix_template = "a close out photo of a small {} and a small {}, {} simple background, pasted, sticker, unnatural, glued, clip-art, attached, collage, low quality."
    ...
    def image_process(self, image, mask, margin=32, fixed_scale=None):
        ...
        if fixed_scale is not None:
            rescaling_factor = min((self.size // 2 - margin) / width, (self.size - margin) / height) * fixed_scale 
        else:
            max_factor = min((self.size // 2 - margin) / width, (self.size - margin) / height)
            rescaling_factor = np.random.uniform(max_factor * 0.9, max_factor)
    ...
    def image_collage(self, image_0, mask_0, image_1, mask_1, white=False, margin_max=10, height_sync=False):
        ...
        mask_map = np.zeros((self.size, self.size))

        if height_sync:
            y_max_sync = self.size - max(image_0.size[1], image_1.size[1])
            y_sync = random.randint(0, y_max_sync) if y_max_sync > 0 else 0
            y_pos_0, y_pos_1 = y_sync, y_sync
        else:
            y_max_0 = self.size - image_0.size[1]
            y_max_1 = self.size - image_1.size[1]
    ...
    def mask_to_tensor(self, mask):
        ...
        float_tensor_map = torch.nn.functional.interpolate(float_tensor_map.unsqueeze(0).unsqueeze(0), size=(self.size // 8, self.size // 8), mode='bilinear')

        return float_tensor_map.squeeze(0) # 1, 128, 128

    def __getitem__(self, index):
        example = {}
        instance_image, mask, caption, category = self.instance_image_captions[index % self.num_instance_images]
        ...
        if do_segmix and self.start_segmix:
            ...
            if args.random_flip and random.random() < 0.5:
                # flip
                additional_instance_image = self.train_flip(additional_instance_image)
                additional_instance_mask = self.train_flip(additional_instance_mask)
            ...
            is_white = random.random() < 0.75
            if is_first:
                merged, merged_mask_map = self.image_collage(image_0, mask_0, image_1, mask_1, white=is_white, margin_max=64, height_sync=False)
            else:
                merged, merged_mask_map = self.image_collage(image_1, mask_1, image_0, mask_0, white=is_white, margin_max=64, height_sync=False)

            example["instance_images"] = self.image_transforms(merged)
            example["instance_prompt"] = self.segmix_template.format(self.id_to_placeholder[category], self.id_to_placeholder[additional_category], 'white' if is_white else 'black')
...
def main(args):
    ...
    train_dataset = DreamBoothDataset(
        # instance_config_path=args.train_config_path,
        # class_config_path=args.reg_config_path,
    ...
    if accelerator.is_main_process:
        tracker_name = "segmix-flux-dev-lora"
        accelerator.init_trackers(tracker_name, config=vars(args), init_kwargs={"wandb": {"name": args.wandb_name}})
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step % args.save_steps == 0:
                        ...
                        FluxPipeline.save_lora_weights(
                            save_directory=os.path.join(args.output_dir, 'step_' + str(global_step)),
